...
describe('SproutSmokeTest',()=>{
    ...
    it('1.11 Company Viewer login and permissions check',()=>{     
        ...
        cy.contains('Cap Table').should('be.exist')  
        cy.contains('Fundraising').should('be.exist')  
        cy.contains('Securities').should('be.exist')  
        cy.wait(2000)
        cy.contains('Data room').click()
        cy.wait(5000)
        cy.contains('Share Class Documents').should('be.exist')
        cy.contains('bylaws').should('be.exist')
        cy.contains('Board').should('be.exist').click()
        cy.get('.data-room-bread-crumb-operation')
          .eq('0')
          .trigger('mouseover')
        cy.wait(2000)
        cy.contains('Open').should('be.exist')
